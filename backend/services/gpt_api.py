...
from azure.core.credentials import AzureKeyCredential
from fastapi import HTTPException
from services.rag_storage import retrieve_similar_reviews, retrieve_similar_pr_comments, store_review
from services.github_reviews import github_service
from typing import Optional
import logging
...
def query_gpt(code_snippet: str, pr_number: Optional[int] = None):
    try:
        ...
        # Step 2: Fetch Top 2 Similar GitHub PR Comments
        pr_comments = retrieve_similar_pr_comments(code_snippet)
        github_context = "\n\n".join(
            f"PR #{comment['pr_number']}: {comment['text']}" for comment in pr_comments
        ) if pr_comments else "No relevant PR comments found."
        ...
        **Code Snippet:**
        ...
        # Process suggested refactored code
        if "suggested_refactored_code" in parsed_response:
            try:
                parsed_response["suggested_refactored_code"] = codecs.decode(parsed_response["suggested_refactored_code"], 'unicode_escape')
            except Exception:
                parsed_response["suggested_refactored_code"] = parsed_response["suggested_refactored_code"].replace('\\n', '\n').replace('\\"', '"')

        # Store final parsed_response
        store_review(parsed_response)

        # Include PR comments in response
        parsed_response["pr_comments"] = pr_comments
...
def followup_query(ai_review, followup_question):
    ...
    {{
        "response": "<Provide a concise and insightful response addressing the user's question.>",
        "suggested_refactored_code": "<Only include this section if the developer's question explicitly asks for or hints at code improvements or implementations. Otherwise, exclude this field from the response.>"
    }}
